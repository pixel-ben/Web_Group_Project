...
function play(){
	if(background.paused){/*does the following if the music is paused*/
		...
		jukebox.src="luc_music.png";/*changes the icon to played*/
	}else{/*does the following if the music is played*/
		background.pause();/*pauses the music*/
		jukebox.src="luc_mute.png";}/*changes the icon to paused*/
}
...
function winlose(){
	...
	boss.src="luc_fight.gif";/*plays the boss fight gif*/
	setTimeout(function() {/*delays the redirect to allow the fade audio and animation play*/
		if (random===0) {/*does the following if the user wins*/
			redirect('luc_win_page.html');/*redirects to the win page*/
		} else {/*does the following if the user loses*/
			redirect('luc_loss_page.html');/*redirects to the win page*/
    }
	}, 3000);
}
...
function restart(){
	...
	fade.style.backgroundImage="url('luc_loading.gif')";/*plays a loading gif*/
	fade.style.backgroundSize="cover";/*covers the full screen with the gif*/
	background.play()/*plays the loading music*/
	setTimeout(function(){/*delays the redirect to allow the fade audio and animation play*/
		window.location.href="luc_home_page.html"/*redirects to the home page*/
	},8000);
}
